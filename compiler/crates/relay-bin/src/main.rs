...
async fn main() {
    ...
    */
    ...
    if (opt.watch || opt.lsp) && !matches!(&config.file_source_config, FileSourceKind::Watchman) {
        panic!(
            "Cannot run relay in watch or LSP mode if `watchman` is not available (or explicitly disabled)."
        );
    }
    ...
    if opt.lsp {
        ...
        debug!("start_language_server");
        if let Err(err) = start_language_server(
            config,
            perf_logger,
            extra_data_provider,
